...
def stgy_handler(top_dir, level, weight_idx, options):
    if level == 0:
        return "random-path --search=nurs:covnew"
    return f"auto --feature={top_dir}/features/{level}.f --weight={top_dir}/weight/{level}/{weight_idx}.w --abstraction-level={options['abstract_level']} --largevalue-threshold={options['lv_hp']}"
...
class klee_executor:
    ...
    def gen_run_cmd(self, level, weight_idx , klee_max_time):
        ...
        search_stgy = stgy_handler(self.top_dir, level, weight_idx, self.options)
